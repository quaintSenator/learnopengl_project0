...
public:
    ...
    void Draw(Shader& shader)
    {
        ...
        {
            meshes[i].Draw(shader, model_format == MODEL_FORMAT_FBX);
        }
    }
    void Draw2Depth(Shader& depthShader, glm::mat4 &model4model)
    {
        depthShader.use();
        depthShader.setMat4("model", model4model);
        for (unsigned int i = 0; i < meshes.size(); i++)
        {
            meshes[i].Draw2depth(depthShader);
        }
    }
...
private:
    ...
    Mesh processMesh(aiMesh* mesh, const aiScene* scene)
    {
        ...
        for (unsigned int i = 0; i < mesh->mNumVertices; i++)
        {
            ...
            if (mesh->mTextureCoords[0]) // does the mesh contain texture coordinates?
            {
                ...
                std::cout << "notice: no texcoords" << std::endl;
            }
        ...
        //如果是obj文件，那么遵循老路，先根据mesh->mMaterialIndex查询material指针
        //然后再从这个指针里面读出maps，转储给mesh.textures
        if(model_format == MODEL_FORMAT_OBJ)
        {
            // 1. diffuse maps
        ...
        else if(model_format == MODEL_FORMAT_FBX)
        {
            SetMeshTextures(textures);
        }
    ...
    void SetMeshTextures(vector<Texture> &texturesToWrite)
    {
        ...
        {
            std::cout<<"PBR assets not complete"<<std::endl;
            return;
        }
        texturesToWrite.clear();
        for(int i = 0; i < 4; i++)
        {
            texturesToWrite.push_back(textures_loaded[i]);
        }
    }
    ...
    void loadGlobalTextures()
    {
        ...
        Paths.push_back(string("Lion_M_Lion_MaskMap.tga"));//R=metallic,G=Occlusion A=smoothness
        Paths.push_back(string("LionGrayscale.tga"));//AO mask
        
        Texture texture;
        for(int i = 0; i < 4; i++)
        {
            texture.id = TextureFromFile(Paths[i].c_str(), this->directory);
            texture.type = "";
            texture.path = "";
