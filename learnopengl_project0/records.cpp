...
//0. Title, includes, tool functions
/*
#include "Shader.h"
#include "camera.h"
#include "Model.h"
...
//2. draw one rectangle with help of EBO
/*int main() {
	glfwInit();
	glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 3);
	glfwWindowHint(GLFW_CONTEXT_VERSION_MINOR, 3);
...
//3. draw triangle which changes color by time
/*
int main() {
	glfwInit();
	glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 3);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		float green_value = (sin(time) + 1.0f) / 2.0f;
		int ourColorLocation = glGetUniformLocation(ourShader.ID, "ourColor");
		glUniform4f(ourColorLocation, 0.0f, green_value, 0.0f, 1.0f);
		//glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);
		glDrawArrays(GL_TRIANGLES, 0, 3);
...
//4. draw triangle with color intersection
/*int main() {
	glfwInit();
	glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 3);
	glfwWindowHint(GLFW_CONTEXT_VERSION_MINOR, 3);
...
/*
 int main() {
	...
	Shader ourShader("./color.fs", "./color.vs");
	...
	std::string filePath = "E:/workplace/vsworkplace/learnopengl_project0/learnopengl_project0/container.jpg";

	//unsigned char* data = stbi_load(filePath.c_str(), &width, &height, &nrChannels, 0);
	unsigned char* data = stbi_load("E:/workplace/vsworkplace/learnopengl_project0/learnopengl_project0/container.jpg", &width, &height, &nrChannels, 0);
...
//6. draw rectangle with 2 textures
/*int main() {
	glfwInit();
	glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 3);
	glfwWindowHint(GLFW_CONTEXT_VERSION_MINOR, 3);
...
//7. draw rectangel with texture, in a 3D level
/*int main() {
	glfwInit();
	glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 3);
	glfwWindowHint(GLFW_CONTEXT_VERSION_MINOR, 3);
	...
	glm::vec4 v1 = glm::vec4(0.5f, 0.5f, 0.0f, 1.0f);
	glm::vec4 v2 = glm::vec4(0.5f, -0.5f, 0.0f, 1.0f);
	glm::vec4 v3 = glm::vec4(-0.5f, -0.5f, 0.0f, 1.0f);
	glm::vec4 v4 = glm::vec4(-0.5f, 0.5f, 0.0f, 1.0f);

	v1 = mvp * v1;
	v2 = mvp * v2;
	v3 = mvp * v3;
	v4 = mvp * v4;
	glm::mat4 va = glm::mat4(v1, v2, v3, v4);
	cout << "vertices Array:" << endl;
	printMat4(glm::value_ptr(va));
...
//9. draw spinning cube, with z-buffer, 2 textures mix
/*
int main() {
	glfwInit();
	glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 3);
	...
	float rotateAngle = sin(glfwGetTime());
	model = glm::rotate(model, 360.0f * rotateAngle, glm::vec3(1.0f, 0.4f, 0.0f));
	//view = glm::translate(view, glm::vec3(0.0f, 0.0f, -3.0f));
...
//11. illumination scene, with a lightCube and a cube, mixed color
/*
* int main() {
	glfwInit();
	glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 3);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		glUniform3fv(glGetUniformLocation(lightShader.ID, "objectColor"), 1, glm::value_ptr(objColor));
...
//12. illumination scene with phong shading
/*
* int main() {
	glfwInit();
	glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 3);
...
//13. better phong-shading box-scene, with diffuse map
/*int main() {
	glfwInit();
	glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 3);
	glfwWindowHint(GLFW_CONTEXT_VERSION_MINOR, 3);
	...
	glm::vec3 lightPos(1.2f, 1.0f, 2.0f);
	std::string filePath1 = "E:/workplace/vsworkplace/learnopengl_project0/learnopengl_project0/container2.png";
	
	unsigned int texture = loadTexture(filePath1.c_str());
	...
	lightShader.setVec3("material.ambient", ma);
	//lightShader.setVec3("material.diffuse", md);
	glUniform1i(glGetUniformLocation(lightShader.ID, "material.diffuse"), 0);
	lightShader.setVec3("material.specular", ms);
	

	glUniform1f(glGetUniformLocation(lightShader.ID, "material.shininess"), shininess);

	lightShader.setVec3("light.ambient", la);
	lightShader.setVec3("light.diffuse", ld);
	lightShader.setVec3("light.specular", ls);
...
//14. box - plane scene without shadowmap
/*
int main()
{
	...
	unsigned int cubeTexture = loadTexture("D:/Cgworkplace/learnopenGLsource/LearnOpenGL/resources/textures/marble.jpg");
	unsigned int floorTexture = loadTexture("D:/Cgworkplace/learnopenGLsource/LearnOpenGL/resources/textures/metal.png");
...
//15. sucess version of nanosuit, full text not just main()
/*
...
int main()
{
	...
	// load models
	// -----------
	std::string filePath = "E:/workplace/vsworkplace/learnopengl_project0/learnopengl_project0/nanosuit/nanosuit.obj";
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		ourShader.use();
		float t = glfwGetTime();
		lightPos.y = 3 * sin(2 * t) + 1;
		lightPos.z = 3 * cos(2 * t);
